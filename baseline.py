...
parser.add_argument(
    "--seed", default=111, type=int, help="seed for initializing training. "
)
...
def main_worker(gpu, args):
    ...
    if args.pretrained:
        ...
        pretrain_path = {
            ...
            "densenet121": "pretrain/densenet121-a639ec97.pth",
        }[args.arch]
    ...
    if args.adam:
        optimizer = torch.optim.Adam(
            model.parameters(), args.base_lr, weight_decay=args.weight_decay
        )
    else:
    ...
    if args.evaluate:
        ...
        if not args.multitask:
            acc, auc, precision_dr, recall_dr, f1score_dr = validate(
            ...
            acc_dr, acc_dme, acc_joint, other_results, se, sp = validate(
                val_loader, model, args
            )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        loss_train = train(
            train_loader, model, criterion, lr_scheduler, writer, epoch, optimizer, args
        )
        ...
        if epoch % 20 == 0:
            if args.dataset == "kaggle":
                ...
                acc_dr, acc_dme, joint_acc, other_results, se, sp = validate(
                    val_loader, model, args
                )
                writer.add_scalar("Val acc_dr", acc_dr, epoch)
        ...
        if args.multitask:
            ...
            if args.crossCBAM:
                ...
                loss = (
                    loss1
                    + loss2
                    + args.lambda_value * loss3
                    + args.lambda_value * loss4
                )
            else:
                loss = loss1 + loss2
        else:
...
def validate(val_loader, model, args):
    ...
    if args.dataset == "kaggle":
        ...
        if args.num_class == 2:
            ...
            precision_dr = precision_score(
                all_target, np.argmax(all_output, axis=1), average="macro"
            )
            recall_dr = recall_score(
                all_target, np.argmax(all_output, axis=1), average="macro"
            )
            f1score_dr = f1_score(
                all_target, np.argmax(all_output, axis=1), average="macro"
        ...
        # joint acc
        joint_result = np.vstack(
            (np.argmax(all_output, axis=1), np.argmax(all_output_dme, axis=1))
        )
        joint_target = np.vstack((all_target, all_target_dme))
        joint_acc = (
            (np.equal(joint_result, joint_target) == True).sum(axis=0) == 2
        ).sum() / joint_result.shape[1]
        ...
        precision_dme = precision_score(
            all_target_dme, np.argmax(all_output_dme, axis=1), average="macro"
        )
