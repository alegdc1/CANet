...
def main_worker(gpu, args):
    ...
    if args.evaluate:
        ...
        if not args.multitask:
            ...
            precision, recall, accuracy, f1 = validate(val_loader, model, args)
            return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % 1 == 0:
            if args.dataset == "kaggle":
                ...
                print("epoch {0}: accuracy: {1}\t auc: {2}".format(epoch, accuracy, f1))
...
def train(train_loader, model, criterion, lr_scheduler, writer, epoch, optimizer, args):
    ...
    for i, (input, target, name) in enumerate(train_loader):
        ...
        if args.multitask:
            ...
            #pdb.set_trace()
            loss = criterion(output.double(), torch.stack(target))
            
            print("loss:", loss)
            #print("target", torch.stack(target))
            #print("Output", output)
            #m = output.double()
            #a = torch.stack(target)
            #accuracy = accuracy_score(a.cpu(), m.cpu())
...
def validate(val_loader, model, args):
    ...
    with torch.no_grad():
        for i, (input, target, name) in enumerate(val_loader):
            ...
            if args.dataset == "ODIR":
                ...
                #pdb.set_trace()
                #print("output0.cpu().data.numpy():", output0.cpu().data.numpy())
                #all_output.append(output0.cpu().data.numpy())
                #all_target_dme.append(target[1].cpu().data.numpy())
                all_output = output
                #for i in range(0,8):
                    #all_output = np.vstack(output[i].cpu().data.numpy())
    ...
    if args.dataset == "kaggle":
        ...
        # maybe torch.from_numpy
        #pdb.set_trace()
        m = torch.sigmoid(all_output)
        m = m > 0.5
        #pdb.set_trace()
        precision, recall, accuracy, f1 = calc_acc_pr_f1_overall(all_target[0], m.cpu())
        print("accuracy" ,accuracy_score(all_target[0], m.cpu()))
